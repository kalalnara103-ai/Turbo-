...
public:
    AudioEffect(audioMasterCallback audioMaster, VstInt32 numP, VstInt32 numParams) {
        ...
        aeffect.magic = 0x56737450;
        aeffect.numPrograms = numP;
        aeffect.numParams = numParams;
        curProgram = 0; // تعريف المتغير اللي مسبب مشكلة
    }
    virtual ~AudioEffect() {}
...
protected:
    AEffect aeffect;
    float sampleRate;
    VstInt32 curProgram; // المتغير هنا
};

class AudioEffectX : public AudioEffect {
public:
    AudioEffectX(audioMasterCallback audioMaster, VstInt32 numP, VstInt32 numParams) 
    : AudioEffect(audioMaster, numP, numParams) {}
